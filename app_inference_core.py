...
class InferenceCore:
    def __init__(self, prop_net:STCN, images, num_objects, top_k=20, mem_every=5, include_last=False, device='cuda'):
    ...
    def encode_key(self, idx):
        result = self.prop_net.encode_key(self.images[:,idx].to(device=self.device))
        return result
        ...
        for ti in this_range:
            ...
            if ti != end:
                ...
                if self.include_last or is_mem_frame:
                    prev_value = self.prop_net.encode_value(self.images[:,ti].to(device=self.device), qf16, out_mask[1:])
    ...
    def interact(self, mask, frame_idx, end_idx):
        mask, _ = pad_divide_by(mask.to(device=self.device), 16)

        self.prob[:, frame_idx] = aggregate(mask, keep_bg=True)

        # KV pair for the interacting frame
        key_k, _, qf16, _, _ = self.encode_key(frame_idx)
        key_v = self.prop_net.encode_value(self.images[:,frame_idx].to(device=self.device), qf16, self.prob[1:,frame_idx].to(device=self.device))
